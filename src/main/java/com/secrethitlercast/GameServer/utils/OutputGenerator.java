...
public class OutputGenerator {
  ...
  private static final ImmutableSet<State> STATES_WITH_CHANCELLOR_CANDIDATE =
      ImmutableSet.of(State.WAITING_FOR_ELECTION);

  private static final ImmutableSet<State> STATES_WITH_PRESIDENT_CANDIDATE =
  ...
  public static String generateOutputForScreen(GameState state, String code) {
    if (state.getState() == State.WAITING_FOR_PLAYERS) {
      return gson.toJson(ScreenOutput.builder().state(state.getState()).code(code)
    ...
    ScreenOutputBuilder builder = ScreenOutput.builder().state(state.getState())
    ...
    if (STATES_WITH_PLAYERS_VOTED.contains(state.getState())) {
      builder.playersVoted(
          state.getVotes().keySet().stream().map(User::getName).collect(toImmutableList()));
    }
    ...
    if (STATES_WITH_GOVERNMENT.contains(state.getState())) {
      builder
          .currentChancellor(state.getCurrentGovernment().get(GovernmentRole.CHANCELLOR).getName())
          .currentPresident(state.getCurrentGovernment().get(GovernmentRole.PRESIDENT).getName());
    }

    if (STATES_WITH_PRESIDENT_CANDIDATE.contains(state.getState())) {
      builder.currentPresidentialCandidate(
          state.getCurrentCandidates().get(GovernmentRole.PRESIDENT).getName());
    }
  ...
  private static String getMessageForState(GameState state) {
    switch (state.getState()) {
      ...
      case WAITING_FOR_CHANCELLOR_LEGISLATION:
        return String.format("Chancellor %s is picking a policy to pass",
            state.getCurrentGovernment().get(GovernmentRole.CHANCELLOR).getName());

      case WAITING_FOR_ELECTION:
        return String.format("Election time, the ballot is President: %s Chancellor: %s",
            state.getCurrentCandidates().get(GovernmentRole.PRESIDENT).getName(),
            state.getCurrentCandidates().get(GovernmentRole.CHANCELLOR).getName());

      case WAITING_FOR_EXECUTION:
        return String.format("President %s will execute somebody.",
            state.getCurrentGovernment().get(GovernmentRole.PRESIDENT).getName());

      case WAITING_FOR_EXECUTIVE_INVESTIGATION:
        return String.format("President %s is investigating somebody's party loyalty.",
            state.getCurrentGovernment().get(GovernmentRole.PRESIDENT).getName());

      case WAITING_FOR_EXECUTIVE_INVESTIGATION_RESULT:
        return String.format("President %s has investigated %s",
            state.getCurrentGovernment().get(GovernmentRole.PRESIDENT).getName(),
            Iterables.getLast(state.getInvestigatedPlayers()).getName());

      case WAITING_FOR_POLICY_PEEK:
        return String.format("President %s is looking at the upcoming agenda",
            state.getCurrentGovernment().get(GovernmentRole.PRESIDENT).getName());

      case WAITING_FOR_PRESIDENTIAL_LEGISLATION:
        return String.format("President %s is picking the agenda",
            state.getCurrentGovernment().get(GovernmentRole.PRESIDENT).getName());

      case WAITING_FOR_PRESIDENTIAL_VETO:
        return String.format("Chancellor %s wants to veto this agenda! Will President %s allow it?",
            state.getCurrentGovernment().get(GovernmentRole.CHANCELLOR).getName(),
            state.getCurrentGovernment().get(GovernmentRole.PRESIDENT).getName());

      case WAITING_FOR_SPECIAL_ELECTION:
        return String.format("President %s is picking his successor.",
            state.getCurrentGovernment().get(GovernmentRole.PRESIDENT).getName());
